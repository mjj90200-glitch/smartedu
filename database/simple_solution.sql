-- ============================================================
-- SmartEdu-Platform 简化方案 SQL 脚本
-- 简化作业提交表结构，支持 AI 全文批改
-- 执行日期：2026-03-24
-- ============================================================
...
-- 插入测试作业（假设已有课程 ID=1，教师 ID=2）
INSERT INTO `homework` (`title`, `description`, `course_id`, `teacher_id`, `attachment_url`, `attachment_name`, `total_score`, `pass_score`, `start_time`, `end_time`, `status`)
VALUES
('第一次作业 - 论述题', '请论述人工智能对未来教育的影响', 1, 2, NULL, NULL, 100, 60, NOW(), DATE_ADD(NOW(), INTERVAL 7 DAY), 1),
('第二次作业 - 分析报告', '阅读指定文档并提交分析报告', 1, 2, NULL, NULL, 100, 60, NOW(), DATE_ADD(NOW(), INTERVAL 14 DAY), 1);

SELECT '简化方案表结构创建成功！' AS message;
SELECT '请执行以下命令测试：1. 学生提交作业 2. 教师 AI 批改' AS next_step;
